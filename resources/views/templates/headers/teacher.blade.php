<header class="bg-[#F2A51A] flex items-center justify-between px-6 py-0 shadow-md">
    ...
    <!-- Info Usuario -->
    <div
        class="flex items-center gap-3 text-right hover:bg-yellow-600 rounded-xl px-3 py-2 transition-colors cursor-pointer">
        <div class="leading-tight">
            <p class="font-bold text-white">{{Auth::user()->name}} {{Auth::user()->last_name}}</p>
            <p class="text-sm text-white">Rol: {{Auth::user()->rol}}</p>
